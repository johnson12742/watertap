...
from electrodialysis_bipolar_expt_deux import (
...
class Test_operation_Method:
    ...
    def bped_model(self):
        ...
        # m.fs.unit.diffus_mass["bpem"].fix((2.03 + 1.96) * 10 ** -9 / 2)
        # m.fs.unit.diffus_mass["bpem"].fix((1.6 + 1.6) * 10**-10 / 2)
        ...
        # m.fs.unit.k_a["bpem"].fix(5e2)
        # m.fs.unit.k_b["bpem"].fix(5e4)
        # m.fs.unit.potential_membrane_bpem[0].fix(1) #[0.2, 0.5, 0.8, 1, 1.2]
    ...
    def test_assign(self, bped_model):
        ...
        print("\n---report_structural_issues display---")
        dt.report_structural_issues()
        print("\n---display_potential_evaluation_errors display---")
        dt.display_potential_evaluation_errors()
        print("\n---report_structural_issues display---")
        dt.report_structural_issues()
        print("\n---display_underconstrained_set display---")
        ...
        m.fs.properties.set_default_scaling(
            "flow_mol_phase_comp", 1e3 * self.factor**-1, index=("Liq", "Na_+")
        )
        m.fs.properties.set_default_scaling(
            "flow_mol_phase_comp", 1e3 * self.factor**-1, index=("Liq", "Cl_-")
        )
        ...
        m.fs.properties.set_default_scaling(
            "flow_mol_phase_comp", 1e3 * self.factor**-1, index=("Liq", "H_+")
        )
        m.fs.properties.set_default_scaling(
            "flow_mol_phase_comp", 1e3 * self.factor**-1, index=("Liq", "OH_-")
        )
        # m.fs.properties.set_default_scaling(
        #     "k_a", 1e-1, index=("bpem")
        # )
        ...
        # iscale.set_scaling_factor(m.fs.unit.aem_side.mass_transfer_term[0.0, "Liq", "OH_-"],
        #                           iscale.get_scaling_factor(m.fs.unit.generation_aem_flux_in[0.0, "Liq", "OH_-"]) *
        #                           iscale.get_scaling_factor(m.fs.unit.cell_width)
        #                           *iscale.get_scaling_factor(m.fs.unit.cell_length)
        #                           *iscale.get_scaling_factor(m.fs.unit.cell_pair_num))
        # iscale.set_scaling_factor(m.fs.unit.cem_side.mass_transfer_term[0.0, "Liq", "H_+"],
        #                           iscale.get_scaling_factor(m.fs.unit.generation_cem_flux_in[0.0, "Liq", "H_+"])*
        #                           iscale.get_scaling_factor(m.fs.unit.cell_width)
        #                           *iscale.get_scaling_factor(m.fs.unit.cell_length)
        #                           *iscale.get_scaling_factor(m.fs.unit.cell_pair_num))
        ...
        print(m.fs.unit.generation_aem_flux_in.display())
        print(m.fs.unit.generation_cem_flux_in.display())
        print(
            "non dim elec field =",
            value(m.fs.unit.elec_field_non_dim[0]),
            pyunits.get_units(m.fs.unit.elec_field_non_dim[0]),
        )
        # print("scaling flux=", iscale.get_scaling_factor(m.fs.unit.flux_splitting[0]))
        # print("flux value=", value(m.fs.unit.flux_splitting[0]),pyunits.get_units(m.fs.unit.flux_splitting[0]))
        # print("scaling flux estimate=", value(m.fs.unit.flux_splitting[0]) * iscale.get_scaling_factor(m.fs.unit.flux_splitting[0]))
        # print("scaling estimate flux k_a =", iscale.get_scaling_factor(m.fs.unit.flux_splitting_k_a) * value(m.fs.unit.flux_splitting_k_a[0]))
        # print("scaling k_a =", iscale.get_scaling_factor(m.fs.unit.k_a) )
        # print("flux k_b =", value(m.fs.unit.flux_splitting_k_b[0]), pyunits.get_units(m.fs.unit.flux_splitting_k_b[0]))
        # print("scaling flux k_b loc=", iscale.get_scaling_factor(m.fs.unit.flux_splitting_k_b) ** -1)
        # print("scaling estimate flux k_b =",
        #       iscale.get_scaling_factor(m.fs.unit.flux_splitting_k_b) * value(m.fs.unit.flux_splitting_k_b[0]))
        # print("scaling k_b =", iscale.get_scaling_factor(m.fs.unit.k_b))
        print(
            "scaling aem_side mass transfer OH- =",
            iscale.get_scaling_factor(
                m.fs.unit.aem_side.mass_transfer_term[0.0, "Liq", "OH_-"]
            ),
        )
        print(
            "Value aem_side mass transfer OH- =",
            value(m.fs.unit.aem_side.mass_transfer_term[0.0, "Liq", "OH_-"]),
        )
        print(
            "Scaling compare aem_side mass transfer OH- =",
            value(m.fs.unit.aem_side.mass_transfer_term[0.0, "Liq", "OH_-"])
            * iscale.get_scaling_factor(
                m.fs.unit.aem_side.mass_transfer_term[0.0, "Liq", "OH_-"]
            ),
        ...
        print(
            "scaling generation_aem_flux OH- =",
            iscale.get_scaling_factor(
                m.fs.unit.generation_aem_flux_in[0.0, "Liq", "OH_-"]
            ),
        )
        print(
            "Value generation_aem_flux OH- =",
            value(m.fs.unit.generation_aem_flux_in[0.0, "Liq", "OH_-"]),
        )
        print(
            "Scaling compare generation_aem_flux OH- =",
            value(m.fs.unit.generation_aem_flux_in[0.0, "Liq", "OH_-"])
            * iscale.get_scaling_factor(
                m.fs.unit.generation_aem_flux_in[0.0, "Liq", "OH_-"]
            ),
        ...
        print(
            "scaling elec_migration_flux_in Na_+ =",
            iscale.get_scaling_factor(
                m.fs.unit.elec_migration_flux_in[0.0, "Liq", "Na_+"]
            ),
        )
        print(
            "Value elec_migration_flux_in Na_+ =",
            value(m.fs.unit.elec_migration_flux_in[0.0, "Liq", "Na_+"]),
        )
        print(
            "Scaling compare elec_migration_flux_in Na_+ =",
            value(m.fs.unit.elec_migration_flux_in[0.0, "Liq", "Na_+"])
            * iscale.get_scaling_factor(
                m.fs.unit.elec_migration_flux_in[0.0, "Liq", "Na_+"]
            ),
        ...
        # print("scaling nonelec_flux_in Na_+ =",
        #       iscale.get_scaling_factor(m.fs.unit.nonelec_flux_in[0.0, "Liq", "Na_+"]))
        # print("Value nonelec_flux_in Na_+ =", value(m.fs.unit.nonelec_flux_in[0.0, "Liq", "Na_+"]))
        # print("Scaling compare nonelec_flux_in Na_+ =",value(m.fs.unit.nonelec_flux_in[0.0, "Liq", "Na_+"])
        #       *iscale.get_scaling_factor(m.fs.unit.nonelec_flux_in[0.0, "Liq", "Na_+"]))
        ...
        print(
            "scaling aem_side mass transfer Na_+ =",
            iscale.get_scaling_factor(
                m.fs.unit.aem_side.mass_transfer_term[0.0, "Liq", "Na_+"]
            ),
        )
        print(
            "Value aem_side mass transfer Na_+ =",
            value(m.fs.unit.aem_side.mass_transfer_term[0.0, "Liq", "Na_+"]),
        )
        print(
            "Scaling compare aem_side mass transfer OH- =",
            value(m.fs.unit.aem_side.mass_transfer_term[0.0, "Liq", "Na_+"])
        ...
        print(iscale.get_scaling_factor(m.fs.unit.eq_generation_aem_flux_in))
        print(
            iscale.get_scaling_factor(
                m.fs.unit.eq_generation_aem_flux_in[0, "Liq", "OH_-"]
            )
        )

        print("---- mass transfer AEM side")
        print(m.fs.unit.aem_side.mass_transfer_term.display())
        print("---- mass transfer CEM side")
        print(m.fs.unit.cem_side.mass_transfer_term.display())
        print("---- Water splitting CEM side")

        # output_current = m.fs.unit.flux_splitting[0]* Constants.faraday_constant+m.fs.unit.current_dens_lim_bpem[0]
        # current_ratio= m.fs.unit.flux_splitting[0]* Constants.faraday_constant/output_current

        # output_current = pyunits.convert(output_current,to_units=pyunits.milliamp ** 3 * pyunits.centimeter ** -2)

        # print("H_+ flux:", value(m.fs.unit.generation_cem_flux_out[0.0, "Liq", "H_+"]), pyunits.get_units(m.fs.unit.generation_cem_flux_out[0.0, "Liq", "H_+"]))
        # print("Total current:", value(output_current), pyunits.get_units(output_current), "\n Generation to total = ", value(current_ratio))
        # print("\n Current generated:", value(output_current) * 0.1, " mA/cm^2 \n Generation to total = ",
        #       value(current_ratio))
        # print("Total current to total current:", value(output_current / m.fs.unit.current[0]* (m.fs.unit.cell_width * m.fs.unit.cell_length)),
        #       pyunits.get_units(output_current / m.fs.unit.current[0]* (m.fs.unit.cell_width * m.fs.unit.cell_length)))
