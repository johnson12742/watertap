...
m.fs.unit.k_a["bpem"].fix(1e3)
m.fs.unit.k_b["bpem"].fix(3e3)
m.fs.unit.potential_membrane_bpem[0].fix(0.1)
# m.fs.unit.membrane_fixed_catalyst["bpem"].fix(5e3)
# m.fs.unit.k_a["bpem"].fix(3e1)
# m.fs.unit.k_b["bpem"].fix(3e1)
# m.fs.unit.potential_membrane_bpem[0].fix(0.1)

# terms = 40
# matrx = 0
# for indx in range(terms):
#     matrx += (2 ** indx * m.fs.unit.elec_field_non_dim[0] ** indx
#               / (math.factorial(indx) * math.factorial(indx + 1)))
#
# matrx *= (m.fs.unit.k2_zero["bpem"] * m.fs.unit.conc_water["bpem"]
#           * m.fs.unit.membrane_fixed_catalyst["bpem"] / m.fs.unit.k_a["bpem"])
#
# matrx *= sqrt(m.fs.unit.potential_membrane_bpem[0]
#               * Constants.vacuum_electric_permittivity
#               * m.fs.unit.relative_permittivity["bpem"]
#               / (
#                       Constants.faraday_constant
#                       * m.fs.unit.membrane_fixed_charge["bpem"]
#               )
#               )
...
m.fs.properties.set_default_scaling(
    "flow_mol_phase_comp", 1e3 * factor**-1, index=("Liq", "H2O")
)
m.fs.properties.set_default_scaling(
...
iscale.set_scaling_factor(m.fs.unit.k_a, 1e-3)
iscale.set_scaling_factor(m.fs.unit.k_b, 1e-3)
# iscale.set_scaling_factor(m.fs.unit.flux_splitting_k_a,1e-1)
# iscale.set_scaling_factor(m.fs.unit.flux_splitting_k_b,1e8)
# print("420")
# @pytest.mark.unit
