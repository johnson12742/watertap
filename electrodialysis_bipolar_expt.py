...
class Electrodialysis0DData(InitializationMixin, UnitModelBlockData):
    ...
    **False** - exclude pressure change terms.}""",
    ...
    # CONFIG.declare(
    #     "salt_membrane_method",
    #     ConfigValue(
    #         default=SaltmembraneMethod.input,
    #         domain=In(SaltmembraneMethod),
    #         description="Configuration for method to obtain salt concentration next to the membrane",
    #         doc="""
    #            **default** - ``SaltmembraneMethod.input``
    #
    #        .. csv-table::
    #            :header: "Configuration Options", "Description"
    #
    #            "``SaltmembraneMethod.input``", "Salt concentration next to the membrane is a variable that the user can input and optimise."
    #            "``SaltmembraneMethod.Empirical``", "Salt concentration next to the membrane based on computed concentration and inlet and outlet."
    #        """,
    #     ),
    # )
    ...
    **Valid values:** {
    ...
    def build(self):
        ...
        if (
            self.config.Operation_method_bpem == LimitingOperationMode.Over_limiting
            or self.config.has_catalyst == True
        ):
            self._make_pre_calculation()
        ...
        def eq_get_velocity_aem_side(self, t):
            # return self.velocity_aem_side[t] == 0 * pyunits.meter * pyunits.second**-1
    ...
    def _make_pre_calculation(self):
        ...
        # @self.Constraint(
        #     self.flowsheet().time,
        #     doc="Evaluate salt concentration on CEM side of bipolar membrane",
        # )
        # def eq_salt_cem_side(self, t):
        #     if self.config.salt_membrane_method == SaltmembraneMethod.Empirical:
        #         cation_conc_cem_loc = 0 * pyunits.mole * pyunits.meter ** -3
        #         anion_conc_cem_loc = 0 * pyunits.mole * pyunits.meter ** -3
        #
        #         for j in self.config.property_package.cation_set:
        #             if not j == "H_+":
        #                 cation_conc_cem_loc += self.cem_side.properties_in[t].conc_mol_phase_comp["Liq", j]
        #
        #         for j in self.config.property_package.anion_set:
        #             if not j == "OH_-":
        #                 anion_conc_cem_loc += self.cem_side.properties_in[t].conc_mol_phase_comp["Liq", j]
        #
        #         conc_unit = 1 * pyunits.mole * pyunits.meter ** -3
        #         return self.salt_conc_cem["bpem"] == smooth_min(cation_conc_cem_loc / conc_unit,
        #                                                         anion_conc_cem_loc / conc_unit) * conc_unit
        #     elif self.config.salt_membrane_method == SaltmembraneMethod.input:
        #         return self.salt_conc_cem["bpem"] == self.config.salt_input_cem * pyunits.mole * pyunits.meter ** -3
        #
        # @self.Constraint(
        #     self.flowsheet().time,
        #     doc="Evaluate salt concentration on AEM side of bipolar membrane",
        # )
        # def eq_salt_aem_side(self, t):
        #     if self.config.salt_membrane_method == SaltmembraneMethod.Empirical:
        #         cation_conc_aem_loc = 0 * pyunits.mole * pyunits.meter ** -3
        #         anion_conc_aem_loc = 0 * pyunits.mole * pyunits.meter ** -3
        #
        #         for j in self.config.property_package.cation_set:
        #             if not j == "H_+":
        #                 cation_conc_aem_loc += self.aem_side.properties_in[t].conc_mol_phase_comp["Liq", j]
        #
        #         for j in self.config.property_package.anion_set:
        #             if not j == "OH_-":
        #                 anion_conc_aem_loc += self.aem_side.properties_in[t].conc_mol_phase_comp["Liq", j]
        #
        #         conc_unit = 1 * pyunits.mole * pyunits.meter ** -3
        #         return self.salt_conc_aem["bpem"] == smooth_min(cation_conc_aem_loc / conc_unit,
        #                                                         anion_conc_aem_loc / conc_unit) * conc_unit
        #     elif self.config.salt_membrane_method == SaltmembraneMethod.input:
        #         return self.salt_conc_aem["bpem"] == self.config.salt_input_aem * pyunits.mole * pyunits.meter ** -3
    ...
    def _make_limiting_operation(self):
        ...
        self.potential_lim_bpem = Var(
            ...
            bounds=(0, 5000),
            units=pyunits.volt,
            doc="Potential barrier across the depletion layeer",
        )
        ...
        def eq_potential_lim_bpem(self, t):
            if self.config.Operation_method_bpem == LimitingOperationMode.Over_limiting:
                if (
                ...
                elif self.config.has_catalyst == True:
                    ...
                    and self.config.has_catalyst == False
                ):
                    #   [H+][OH-] concentration
    ...
    def _make_catalyst(self):
        ...
        self.flux_splitting = Var(
            ...
            # bounds=(0, 50000),
            units=pyunits.mole * pyunits.meter**-2 * pyunits.second**-1,
            doc="Flux generated via k_a",
        )
        # self.flux_splitting_k_a = Var(
        #     self.flowsheet().time,
        #     initialize=1,
        #     domain=NonNegativeReals,
        #     # bounds=(0, 50000),
        ...
        #     doc="Flux generated via k_a",
        # )
        # self.flux_splitting_k_b = Var(
        #     self.flowsheet().time,
        #     initialize=1,
        #     domain=NonNegativeReals,
        #     # bounds=(0, 50000),
        #     units=pyunits.mole * pyunits.meter ** -2 * pyunits.second ** -1,
        #     doc="Flux generated via k_b",
        # )
        ...
        def eq_potential_membrane_bpem_non_dim(self, t):
            # return self.elec_field_non_dim[t] == 3
            # return self.elec_field_non_dim[t] == 0.1/2.3240739600041578e-11
            ...
            # return (self.elec_field_non_dim[t] * self.relative_permittivity["bpem"]
            #         * self.aem_side.properties_in[t].temperature ** 2 / const) == sqrt((Constants.faraday_constant
            #             * self.membrane_fixed_charge["bpem"]
            #             * self.potential_membrane_bpem[t])/(Constants.vacuum_electric_permittivity*self.relative_permittivity["bpem"]))
            # return ((self.elec_field_non_dim[t] * self.relative_permittivity["bpem"] *
            #          self.aem_side.properties_in[t].temperature ** 2 / const) ** 2 *
            #         (Constants.vacuum_electric_permittivity * self.relative_permittivity["bpem"] /
        ...
        def eq_flux_splitting(self, t):
            ...
            # matrx *= (self.k2_zero["bpem"] * self.conc_water["bpem"]
            #           * self.membrane_fixed_catalyst_cem["bpem"] / self.k_a["bpem"])

            # matrx *= sqrt(self.potential_membrane_bpem[t]
            #               * Constants.vacuum_electric_permittivity
            #               * self.relative_permittivity["bpem"]
            #               / (
            #                       Constants.faraday_constant
        ...
        #     doc="Calculate the potential barrier at limiting current across the bipolar membrane",
        # )
        # def eq_flux_splitting_k_a(self, t):
        #     terms = 40
        #     matrx = 0
        #     for indx in range(terms):
        #         matrx += (2 ** indx * self.elec_field_non_dim[t] ** indx
        #                 / (math.factorial(indx) * math.factorial(indx + 1)) )
        #
        #     matrx *= (self.k2_zero["bpem"] * self.conc_water["bpem"]
        #               * self.membrane_fixed_catalyst_cem["bpem"]/self.k_a["bpem"])
        ...
        #         )
        # )
        #     return self.flux_splitting_k_a[t] == matrx
        #
        # @self.Constraint(
        #     self.flowsheet().time,
        #     doc="Calculate the potential barrier at limiting current across the bipolar membrane",
        # )
        # def eq_flux_splitting_k_b(self, t):
        #     terms = 40
        #     matrx = 0
        #     for indx in range(terms):
        #         matrx += (2 ** indx * self.elec_field_non_dim[t] ** indx
        ...
        #
        #     matrx *= (self.k2_zero["bpem"] * self.conc_water["bpem"]
        #               * self.membrane_fixed_catalyst_aem["bpem"] / self.k_b["bpem"])
        #
        #     matrx *= sqrt(self.potential_membrane_bpem[t]
        #                   * Constants.vacuum_electric_permittivity
        #                   * self.relative_permittivity["bpem"]
        ...
        #                   )
        #                   )
        #     return self.flux_splitting_k_b[t] == matrx
    ...
    def calculate_scaling_factors(self):
        ...
        if (
            ...
            if iscale.get_scaling_factor(self.conc_water, warning=True) is None:
        ...
        if (
            ...
            if iscale.get_scaling_factor(self.k2_zero, warning=True) is None:
        ...
        if self.config.has_catalyst == True:
            if iscale.get_scaling_factor(self.flux_splitting, warning=True) is None:
                ...
                #     * value(Constants.vacuum_electric_permittivity) ** -1
                #     * iscale.get_scaling_factor(self.relative_permittivity)
                #     /(iscale.get_scaling_factor(self.membrane_fixed_charge) * value(Constants.faraday_constant) **-1
                #        )
                #     * iscale.get_scaling_factor(self.membrane_fixed_catalyst) * iscale.get_scaling_factor(self.k_a) ** -1
                #     * iscale.get_scaling_factor(self.k2_zero) * iscale.get_scaling_factor(self.elec_field_non_dim) *
                #     iscale.get_scaling_factor(self.conc_water)
                # )
                iscale.set_scaling_factor(self.flux_splitting, sf)

            # if(iscale.get_scaling_factor(self.flux_splitting_k_b, warning=True) is None):
            #
            #     terms = 40
            #     sf = 0
            #     for indx in range(terms):
            #         sf += (2 ** indx * iscale.get_scaling_factor(self.elec_field_non_dim) ** -indx
            #                / (math.factorial(indx) * math.factorial(indx + 1)))
            #
            #     sf **= -1
            #     sf *= (iscale.get_scaling_factor(self.k2_zero) * iscale.get_scaling_factor(self.conc_water) *
            #            iscale.get_scaling_factor(self.membrane_fixed_catalyst_aem) / iscale.get_scaling_factor(self.k_b))
            #
            #     sf *= sqrt(iscale.get_scaling_factor(self.potential_membrane_bpem)
            #                * value(Constants.vacuum_electric_permittivity) ** -1
            #                * iscale.get_scaling_factor(self.relative_permittivity)
            #                / (
            #                        value(Constants.faraday_constant) ** -1
            ...
            #
            #
            #     iscale.set_scaling_factor(self.flux_splitting_k_b, sf)
        ...
        for ind in self.generation_aem_flux_in:
            if ind[2] == "OH_-":
                if self.config.has_catalyst == True:
                    sf = sf = iscale.get_scaling_factor(self.flux_splitting)
                elif (
                    self.config.Operation_method_bpem
                    == LimitingOperationMode.Over_limiting
                ):
        ...
        # iscale.set_scaling_factor(self.aem_side.mass_transfer_term[0.0, "Liq", "OH_-"],
        #                           iscale.get_scaling_factor(self.generation_aem_flux_in[0.0, "Liq", "OH_-"]) *
        ...
        # iscale.set_scaling_factor(self.cem_side.mass_transfer_term[0.0, "Liq", "H_+"],
        #                           iscale.get_scaling_factor(self.generation_cem_flux_in[0.0, "Liq", "H_+"]) *
        #                           iscale.get_scaling_factor(self.cell_width)
        #                           * iscale.get_scaling_factor(self.cell_length)
        #                           * iscale.get_scaling_factor(self.cell_pair_num))
        ...
        if hasattr(self, "current_dens_lim_bpem"):
            if iscale.get_scaling_factor(self.current_dens_lim_bpem) is None:
                if (
                    ...
                    # if (self.config.salt_membrane_method == SaltmembraneMethod.input):
                    sf = (
                    ...
                    # elif(self.config.salt_membrane_method
                    #     == SaltmembraneMethod.Empirical):
                    #     sf = (
                    #             iscale.get_scaling_factor(self.diffus_mass)
                    #             * value(Constants.faraday_constant) ** -1
                    #             * (2 * 1e-2) ** 2
        ...
        if hasattr(self, "eq_potential_membrane_bpem_non_dim"):
            for ind, c in self.eq_potential_membrane_bpem_non_dim.items():
                print(
                    ...
                    ind,
                    "scaling factor:",
                    iscale.get_scaling_factor(self.elec_field_non_dim[ind]),
                )
                iscale.constraint_scaling_transform(
        ...
        for ind, c in self.eq_mass_transfer_term_cem_side.items():
            # print("what's c :", c, "index:", ind, "scaling factor:", min(
            #         iscale.get_scaling_factor(self.generation_aem_flux_in[ind]),
            #         iscale.get_scaling_factor(self.elec_migration_flux_in[ind]),
            #         iscale.get_scaling_factor(
            #             self.nonelec_flux_in[ind], self.elec_migration_flux_out[ind]
            #         ),
            #         iscale.get_scaling_factor(self.nonelec_flux_out[ind]),
            #     )* iscale.get_scaling_factor(self.cell_width) * iscale.get_scaling_factor(self.cell_length)*
            #     iscale.get_scaling_factor(self.cell_pair_num))
            iscale.constraint_scaling_transform(
                c,
                min(
