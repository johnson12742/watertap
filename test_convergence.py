...
kd_zero = 2 * 10**-5 * pyunits.second**-1
...
conc_salt = 1 * (0.5 + 0.25 * 2) * pyunits.mol * pyunits.L**-1
slit_length = 130e-6 * pyunits.meter
...
potential_membrane_bpem = 0.6 * pyunits.volt
...
print(
    f" Non dim elec field  = {value(elec_field_non_dim):.3e}, {pyunits.get_units(elec_field_non_dim)}"
)

elec_dim = const**-1 * elec_field_non_dim * temperature**2 * relative_permittivity**1

print(
    f" dimensional elec field  = {value(elec_dim):.3e}, {pyunits.get_units(elec_dim)}"
)
...
terms = 40
matrx = 0
k_a = 1e-3 * pyunits.mole * pyunits.meter**-3
...
fe_val = matrx
print(f" f(E)  = {value(fe_val):.3e}, {pyunits.get_units(fe_val)}")
print(
...
print(
    f" depletion length catalyst  ={value(depletion_length):.3e}, {pyunits.get_units(depletion_length)}"
)
...
for indx in range(terms):
    ...
    matrx_scale += (
        2**indx
        * elec_field_non_dim_scale**-indx
        / (math.factorial(indx) * math.factorial(indx + 1))
    )

matrx_scale **= -1
print(f" f(E) scale = {value(matrx_scale):.3e}, {pyunits.get_units(matrx_scale)}")
fe_val_scale = matrx_scale

matrx_scale *= (
    k2_zero_scale * conc_water_deux_scale * membrane_fixed_catalyst_scale / k_a_scale
)

print(f" Rate scale = {value(matrx_scale):.3e}, {pyunits.get_units(matrx_scale)}")

matrx_scale *= sqrt(
    potential_membrane_bpem_scale
    * value(Constants.vacuum_electric_permittivity) ** -1
    * relative_permittivity_scale
    / (value(Constants.faraday_constant) ** -1 * membrane_fixed_charge_scale)
)

print(f" flux scale = {value(matrx_scale):.3e}, {pyunits.get_units(matrx_scale)}")

print(f" f(E) scaling estimate = {value(fe_val_scale)*value(fe_val):.3e}")
