...
class Test_operation_Method:
    ...
    def bped_model(self):
        ...
        m.fs.unit.potential_membrane_bpem[0].fix(0.8)  # [0.2, 0.6, 0.8, 1, 1.2]
    ...
    def test_assign(self, bped_model):
        ...
        print(
            "flux value=",
            value(m.fs.unit.flux_splitting[0]),
            pyunits.get_units(m.fs.unit.flux_splitting[0]),
        )
        print(
            "scaling flux estimate=",
            value(m.fs.unit.flux_splitting[0])
            * iscale.get_scaling_factor(m.fs.unit.flux_splitting[0]),
        )
        ...
        output_current = (
            m.fs.unit.flux_splitting[0] * Constants.faraday_constant
            + m.fs.unit.current_dens_lim_bpem[0]
        )
        current_ratio = (
            m.fs.unit.flux_splitting[0] * Constants.faraday_constant / output_current
        )
        ...
        print(
            ...
            pyunits.get_units(output_current),
            "\n Generation to total = ",
            value(current_ratio),
        )
        print(
            "\n Current generated:",
            value(output_current) * 0.1,
            " mA/cm^2 \n Generation to total = ",
            value(current_ratio),
        )
        print(
            "Total current to total current:",
            value(
                output_current
                / m.fs.unit.current[0]
